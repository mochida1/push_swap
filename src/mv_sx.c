...
/*   Updated: 2022/06/20 20:05:38 by hmochida         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	swap_stack_b(t_pushswap_data *ps_data, t_movements *mv_to_print)
{
	...
	add_to_mv(SWAP_B, ps_data->mv_to_print);
}
...
void	swap_stack_a(t_pushswap_data *ps_data, t_movements *mv_to_print)
{
	...
	add_to_mv(SWAP_A, ps_data->mv_to_print);
}

/*
** makes the swap movent on both lists and saves it to mv_to_print.
*/
void	swap_stack_s(t_pushswap_data *ps_data, t_movements *mv_to_print)
{
	swap_stack_a(ps_data, mv_to_print);
	swap_stack_b(ps_data, mv_to_print);
	add_to_mv(SWAP_S, ps_data->mv_to_print);
}